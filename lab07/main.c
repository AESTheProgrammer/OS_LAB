...
int main(int argc, char const *argv[])
{
    ...
    for (int i = 0; i < NUMBER_OF_CUSTOMERS; i++)
    {
        for (int j = 0; j < NUMBER_OF_RESOURCES; j++)
        {
            ...
            need[i][j] = maximum[i][j]; // need = maximum - allocated and allocated is initialized to 0
        }
    }
    print_state();
    ...
    for (int i = 0; i < NUMBER_OF_RESOURCES; i++)
    {
        pthread_create(&customer_threads[i], NULL, (void *) customer_thread, NULL);
    }
    for (int i = 0; i < 5; i++)
    {
...
void *customer_thread(int n) {
    ...
    {
        request_release = rand() % 2;
        if (request_release == 1) { // request
            for (int j = 0; j < NUMBER_OF_RESOURCES; j++)
            {
